...
@pytest.mark.asyncio
async def test_first_scan(test_hub: Hub, mocker: MockFixture):
    assert test_hub.agent
    secret = 0xdeadbeefcafebabe
    expected_address = await test_hub.agent.setup_scan_test(secret)
    address_count = await test_hub.agent.scan(secret, ScanSize.U64.value, ScanType.EXACT.value)
